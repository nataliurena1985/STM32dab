...
HAL_StatusTypeDef SPI_TransmitReceive(SPI_HandleTypeDef *hspi, uint8_t Data);


extern SPI_HandleTypeDef hspi3;
extern uint8_t Timer1, Timer2;                    /* 10ms 마다 감소하는 타이머 */

extern DSTATUS Stat;
extern BYTE CardType;

//static
uint8_t PowerFlag = 0;                           /* Power 상태 Flag */
...
DSTATUS SD_disk_initialize(BYTE drv) 
{
  ...
  if (SD_SendCmd(CMD_0, 0) == 1)
  { 
    Timer1 = 100;
    if (SD_SendCmd(CMD_8, 0x1AA) == 1)
    { 
      for (n = 0; n < 4; n++)
      {
        ocr[n] = SPI_RxByte();
      ...
      { 
        /* 2.7-3.6V 전압범위 동작 */
        do {
          if (SD_SendCmd(CMD_55, 0) <= 1 && SD_SendCmd(CMD_41, 1UL << 30) == 0)
            break; /* ACMD_41 with HCS bit */
        } while (Timer1);
        
        if (Timer1 && SD_SendCmd(CMD_58, 0) == 0)
        { 
          /* Check CCS bit */
          for (n = 0; n < 4; n++)
          {
    ...
    { 
      /* SDC Ver1 or MMC */
      type = (SD_SendCmd(CMD_55, 0) <= 1 && SD_SendCmd(CMD_41, 0) <= 1) ? 2 : 1; /* SDC : MMC */
      
      do {
        if (type == 2) 
        {
          if (SD_SendCmd(CMD_55, 0) <= 1 && SD_SendCmd(CMD_41, 0) == 0)
            break; /* ACMD41 */
        } 
        else 
        {
          if (SD_SendCmd(CMD_1, 0) == 0)
            break; /* CMD_1 */
        }
      } while (Timer1);
      
      if (!Timer1 || SD_SendCmd(CMD_16, 512) != 0)
      {
        type = 0;
      }
    }
